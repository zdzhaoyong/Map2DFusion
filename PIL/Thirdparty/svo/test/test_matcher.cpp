...
void MatcherTest::testEpipolarSearchFullImg()
{
  ...
  for(int y=0,yend=depth_ref_.rows;y<yend;y++)
  for(int x=0,xend=depth_ref_.cols;x<xend;x++)
  {
      depth_ref_.at<float>(y,x)*=0.1;
      depthmap.at<float>(y,x)*=0.1;
      error_map.at<float>(y,x)*=0.1;
  }
