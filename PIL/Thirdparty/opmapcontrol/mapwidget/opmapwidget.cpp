...
void OPMapWidget::SetShowCircularFlight(const bool &value)
{
    if( value && CFI == 0 ) {
        CFI = new CircularFlightItem(map, this);
        CFI->setParentItem(map);

        showCircularFlight = value;
    } else if( !value ) {
        if(CFI != 0) {
            delete CFI;
            CFI = 0;
